...
async def upload_file(file: UploadFile = File(...)):
    try:
        ...
        python_executable = sys.executable  # <-- This picks the current venv python executable
        # subprocess.Popen([python_executable, "-m", "ingestion.ingest", "--clean"])
...
async def upload_text(data: dict = Body(...)):
    """
    Accept job description text, save it to a file, and trigger ingestion.
    """
    try:
        ...
        # Create a unique filename (or fixed name if you want)
        filename = f"job_description_{uuid.uuid4().hex}.md"
        ...
        # Trigger ingestion process (same as your upload endpoint)
...
if os.path.exists(static_dir):
    ...
    async def serve_chat_interface():
        ...
        if os.path.exists(index_path):
            ...
            return {"message": "Chat interface not found. Please add index.html to the static directory."}
